...
def process_result_html(
    root: lxml.html.HtmlElement, student: Student, exam_descriptor: ExamDescriptor
) -> ExamReport:
    ...
    score = turkish_str_to_float(
        root.xpath("./div[1]/div[3]/div/div/div/div[2]")[0].text_content()
    )
    ...
    for i in range(2, 7):
        for j in (5, 6):
            ranks_and_attendances.append(
                int(
                    root.xpath(f"./div[1]/div[{j}]/div/div/div/div[{i}]")[0]
                    .text_content()
                    .split("%")[0]
                    .rstrip()
                )
    ...
    for test_d in exam_descriptor.test_descriptors:
        ...
        net = turkish_str_to_float(
            test_root.xpath("./div[5]/div/div/*[last()]")[0].text_content()
        )
